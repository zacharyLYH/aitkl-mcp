...
""" 
Step 1: This is given to you for free. Just uncomment the code below and it should work.
Step 2: Try calling the api in "Example" in your browser and observe its output - see if you can understand how the code works and ultimately returns the data.

We're going a step beyond and also using an LLM in the frontend to summarise the data. 
- In client.py/_execute_tool_call(), after calling the tool, we call send_tool_result() with whatever you prompt you return from this function.
- The purpose of this is so our app responds in natural language, rather than just the raw data (which kinda breaks user experience?).
"""
@mcp.tool()
...
"""
This function doesn't need any modification. Your work is in get_weather_forecast()
"""
@mcp.tool()
async def get_weather_by_location(location: str, days: int = 7) -> str:
    ...
    """
    coords = await get_coordinates_for_location(location) #This is a provided function. No need to change it.
...
"""
Step 1: Observe the output of the print. You'll need to understand the response shape for this task
Step 2: Enrich the "returned_prompt" with the weather data you got from the API call. The goal is to provide as much information about the weather forecast of this lat/lon combo so that gemini has more to work with. Garbage in, garbage out.
"""
async def get_weather_forecast(latitude: float, longitude: float, days: int = 7) -> str:
    ...
    log.info(f"!!!forecast api response: {data}")
    
    current = data.get('current_weather', {})
    daily = data.get('daily', {})
    
    returned_prompt = "Summarise this for me. Do not modify or add information. Weather Forecast:\n\n"
    
    # Current weather
    if current:
        returned_prompt += f"Current Weather ({current.get('time', 'Unknown')}):\n"
        """
        Try your hand at adding more information to the returned_prompt.
        """
    
    # Daily forecast
    if daily and 'time' in daily:
        """
        If you're up for it, return the daily forecast in detail too!
        """
    
    return returned_prompt

"""
I think you're getting the hang of it. Your goal is to study the output of the api call and return a prompt that is as rich as possible.
"""
@mcp.tool()
async def get_public_holidays(year: int, country_code: str) -> str:
    ...
    returned_prompt = "" #Extract data from "data" and enrich this prompt.
    
    return returned_prompt

"""
Rinsing and repeating. However, this is more challenging. This API returns more information - you'll need to study the output and return a prompt that is as rich as possible.
"""
@mcp.tool()
async def get_country_info(country_name: str) -> str:
    ...
    returned_prompt = "" #Extract data from "country" and enrich this prompt.

    return returned_prompt

"""
Another challenging tool. You got this.
"""
@mcp.tool()
...
"""
We're bringing it all together. There's no work for you here. Go ahead and query this tool using prompt 6 (last section) from the README.
"""
@mcp.tool()
...
"""
We're bringing it all together. There's no work for you here. Go ahead and query this tool using prompt 7 (last section) from the README.
"""
@mcp.tool()
